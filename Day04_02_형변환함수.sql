/*
    DUAL 테이블
    1. DUMMY 칼럼(이거 딱 하나 있음) 1개 + 'X' 값을 1개 가지고 있는 테이블이다.
    2. 아무 일도 안하는 테이블이다.
...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
  FROM EMPLOYEES
 WHERE EMPLOYEE_ID = '150'; --EMPLOYEE_ID = TO_NUMBER('150')으로 자동으로 변환된 뒤 처리한다.
...
SELECT
     ...
     ,SYSTIMESTAMP      --TIMESTAMP 타입의 현재 날짜와 시간, 좀 더 정밀한 시간을 원할 때 이용
  FROM
      DUAL;
