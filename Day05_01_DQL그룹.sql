/*
    GROUP BY절(GROUP BY절에 있는 칼럼은 SELECT 절에 꼭들어가야 하지만 반대조건은 성립하지 않는다)
    1. GROUP BY절에서 지정한 칼럼의 데이터는 동일한 데이터끼리 하나로 모여서 조회된다.
    2. GROUP BY절에서 지정한 칼럼은 "반드시" GROUP BY절에 있어야 한다.(필수규칙) 
*/

/* DEPARTMENT_ID은 묶이지만 FIRST_NAME은 안묶이므로 이건 실행이 안된다
...
--3. GROUP BY절이 없는 집계함수는 전테 데이터를 대상으로 한다.
SELECT 
        COUNT(*) AS 전체사원수
...
/*
    조건 지정하기
    1. GROUP BY절로 그룹화 할 대상(모수)이 적을수록 성능이 유리하다
    2. GROUP BY 이전에 처리할 수 있는 조건은 WHERE절로 처리하는 것이 유리하다.(둘다 쓸수 있는 상황이면 이게 더 성능이 좋다)
