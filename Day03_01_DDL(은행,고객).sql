/*
    ...
    4. **완료된 작업을 취소할 수 없다.**(COMMIT할 필요가 없다. ROLLBACK을 할 수 없다, 저장도 안되고 취소도 안된다.)
    5. 종류
        1) CREATE : 생성
        2) ALTER  : 수정
        3) DROP   : 삭제
...
CREATE TABLE CUSTOMER_TBL (   
    ...
    CONSTRAINT PK_CUSTOMER_TBL    PRIMARY KEY (NO),
...
--      GRADE 칼럼은 ('VIP', 'GOLD', 'SILVER', 'BRONZE' 중 하나의 값만 가질 수 있도록 
ALTER TABLE CUSTOMER_TBL
    ADD GRADE VARCHAR2(6 BYTE) CHECK(GRADE IN('VIP', 'GOLD', 'SILVER', 'BROZE'));
--6. BANK_TBL 테이블의 BANK_NAME 칼럼에 NOT NULL 제약조건을 추가하시오.
ALTER TABLE BANK_TBL
    MODIFY BANK_NAME VARCHAR2(15 BYTE) NOT NULL;  --데이터 타입의 변경이 없다면 전에 작성된 자료를 보고 그대로 작성해줘야한다.
...
ALTER TABLE CUSTOMER_TBL
    MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL;  --생성할땐 안적어주면 NULL이지만 수정 할때는 안적어주면 이전 제약조건을 그대로 쓴다는 의미다, NULL 명시를 반드시 해야 한다.
